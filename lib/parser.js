...
import formatHTMLStructure from "./htmlFormatter.js";
import fs from "fs";
...
const parse = (markdown, onPort, isDebugMod) => {
    ...
    parsedCode = parsedCode
        .replace(/^@css (.*$)/gm, (style) => {
            ...
            const filePath = path.join(path.dirname(fileURLToPath(import.meta.url)), `../public/${styleName}.css`); // It is same as path.join(__dirname, file_path_to_css)
    ...
    server(htmlCode, onPort, isDebugMod);
}
